...
typedef NS_ENUM(NSInteger, EventPriority) {
    HighPriority,
    NormalPriority,
};

typedef NS_ENUM(NSInteger, ReportingStrategy) {
    ForceSync,
    Buffer,
};
...
- (void)trackEventWithName:(NSString *)eventName andValue:(NSDictionary *)eventValue priority:(enum EventPriority)eventPriority;


- (void)trackSystemEventWithName:(NSString *)eventName andValue:(NSDictionary *)eventValue;

- (void)forceTrackSDKEventWithName:(NSString *)eventName andValue:(NSDictionary *)eventData DEPRECATED_MSG_ATTRIBUTE("Please use trackEventWithName() with the priority parameter.");
/**
 *  Track an event in WebEngage, an event represents an action taken by user like clicking a button, adding items to cart, searching etc. Its upto the application to decide the names and data for these events.
 *  @see [WebEngage Docs](http://docs.webengage.com/overview/readme.html#event) and [WebEngage Docs iOS](http://docs.webengage.com/sdks/ios/events/readme.html)
 *
...
- (void)trackEventWithName:(NSString *)eventName priority:(enum EventPriority)eventPriority;
