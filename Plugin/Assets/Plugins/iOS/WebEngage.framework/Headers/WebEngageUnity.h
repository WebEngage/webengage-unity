...
extern "C" {
    ...
    void loginWithJWT(const char* cuid,const char* jwt);

    void setSecureToken(const char* cuid, const char* jwt);
    ...
    void autoTrackUserLocationWithAccuracy(int accuracy);

    void setProxyURL(const char* proxyURL);

    void resetProxyURL(void);

    const char* getDeeplinkFor(const char* inAppNotificationData, const char* actionId);
    ...
    void jwtTokenInvalidatedCallBack(notificationCallBack callback);
